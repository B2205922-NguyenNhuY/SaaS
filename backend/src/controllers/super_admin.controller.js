...
exports.createSuperAdmin = async (req, res) => {
  try {
    // 1. Chuẩn hóa role của người đang thực hiện request
    ...
    // 2. Chuẩn hóa role mục định muốn tạo
    ...
    const { email, password, hoTen, soDienThoai, trangThai } = req.body;
...
exports.getAllSuperAdmins = async (req, res) => {
  try {
    const rows = await superAdminModel.getAllSuperAdmins(); // Bỏ dấu []
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
exports.updateSuperAdminInfo = async (req, res) => {
  try {
    ...
    const duplicate = await superAdminModel.checkDuplicateForUpdate(
      id,
      email,
      soDienThoai,
    );
