...
async function getCurrentPlan(tenant_id) {
  const [rows] = await db.query(`SELECT p.* FROM plan_subscription ps JOIN plan p ON ps.plan_id = p.plan_id WHERE ps.tenant_id = ? AND ps.trangThai = 'active' AND ps.ngayKetThuc > NOW() ORDER BY ps.subscription_id DESC LIMIT 1`, [tenant_id]);
  return rows[0] || null;
}

async function countKiosks(tenant_id) {
  const [[row]] = await db.query(`SELECT COUNT(*) AS total FROM kiosk WHERE tenant_id = ?`, [tenant_id]);
...
exports.create = async (tenant_id, body) => {
  ...
  if (!plan) {
    throw Object.assign(new Error("Subscription không hợp lệ hoặc đã hết hạn"), { statusCode: 403 });
  }
  const totalKiosks = await countKiosks(tenant_id);
  if (totalKiosks >= Number(plan.gioiHanSoKiosk || 0)) {
    throw Object.assign(new Error("Đã vượt quá số lượng kiosk cho phép của gói"), { statusCode: 400 });
  }
