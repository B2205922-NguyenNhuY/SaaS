...
async function existsCCCD(tenant_id, CCCD, excludeId = null) {
  const params = [tenant_id, CCCD];
  let sql = `SELECT merchant_id FROM merchant WHERE tenant_id = ? AND CCCD = ?`;
  if (excludeId) {
    sql += ` AND merchant_id <> ?`;
...
exports.create = async (tenant_id, body) => {
  ...
  if (await existsCCCD(tenant_id, CCCD)) {
...
exports.update = async (tenant_id, merchant_id, body) => {
  const CCCD = body.CCCD != null ? String(body.CCCD).trim() : null;
  if (CCCD && (await existsCCCD(tenant_id, CCCD, merchant_id))) {
    throw Object.assign(new Error("CCCD already exists in this tenant"), {
      statusCode: 409,
    });
  }
