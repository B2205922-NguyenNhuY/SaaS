...
async function assertMarketQuota(tenant_id) {
  ...
  if (!plan) return;
  const [[{ total }]] = await db.query('SELECT COUNT(*) total FROM market WHERE tenant_id = ?', [tenant_id]);
  if (Number(total) >= Number(plan.gioiHanSoCho || 0)) {
...
async function assertKioskQuota(tenant_id) {
  const plan = await getActivePlan(tenant_id);
  if (!plan) return;
  const [[{ total }]] = await db.query('SELECT COUNT(*) total FROM kiosk WHERE tenant_id = ?', [tenant_id]);
  if (Number(total) >= Number(plan.gioiHanSoKiosk || 0)) {
...
async function assertUserQuota(tenant_id) {
  const plan = await getActivePlan(tenant_id);
  if (!plan) return;
  const [[{ total }]] = await db.query("SELECT COUNT(*) total FROM users WHERE tenant_id = ? AND trangThai <> 'deleted'", [tenant_id]);
  if (Number(total) >= Number(plan.gioiHanUser || 0)) {
...
module.exports = { getActivePlan, assertMarketQuota, assertKioskQuota, assertUserQuota };
