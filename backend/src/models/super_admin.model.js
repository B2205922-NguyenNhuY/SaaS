...
// Cập nhật hàm createSuperAdmin trong models/super_admin.model.js
exports.createSuperAdmin = async (data) => {
  const { email, password_hash, soDienThoai, hoTen, trangThai } = data;
...
exports.getAllSuperAdmins = async () => {
  const [rows] = await db.execute("SELECT * FROM super_admin");
...
exports.updateSuperAdminInfo = async (id, data) => {
  ...
  const [result] = await db.execute(
    ...
    [email ?? null, soDienThoai ?? null, hoTen ?? null, id],
  );
  return result;
};
