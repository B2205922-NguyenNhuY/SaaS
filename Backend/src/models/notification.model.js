...
exports.markAsRead = async (notification_id, user_id, tenant_id, role) => {
  const [result] = await db.execute(
    `INSERT INTO notification_read (notification_id, user_id, tenant_id, type)
         VALUES (?, ?, ?, ?)`,
    [notification_id, user_id, tenant_id, role],
  );
  return result;
};
