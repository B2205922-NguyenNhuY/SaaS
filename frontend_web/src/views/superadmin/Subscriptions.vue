  ...
  <div class="sub-page">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Lịch sử gán gói cước và thanh toán của Tenant</p>
      </div>
    </div>
    ...
    <div class="filter-bar">
      <div class="search-wrap">
        ...
        <input v-model="filters.keyword" placeholder="Tìm theo tên tenant, plan..." @input="debounce" />
      </div>
      ...
      <select v-model="filters.plan_id" @change="fetch">
        ...
        <option v-for="p in plans" :key="p.plan_id" :value="p.plan_id">{{ p.tenPlan }}</option>
      </select>
      <input type="date" v-model="filters.start_date" @change="fetch" class="date-input" title="Từ ngày" />
      <input type="date" v-model="filters.end_date" @change="fetch" class="date-input" title="Đến ngày" />
    </div>
    ...
    <div class="table-panel">
        ...
        <table class="tbl">
          ...
          <tbody>
            ...
            <tr v-for="s in subs" :key="s.subscription_id" class="tbl-row">
              <td>
                <div class="cell-with-av">
                  ...
                  <div>
                    ...
                    <div class="cell-sub">ID #{{ s.tenant_id }}</div>
                  </div>
                </div>
              </td>
              <td>
                <span class="plan-tag">{{ s.tenGoi }}</span>
              </td>
              <td>
              ...
              <td class="cell-date">
                <span :class="{ 'expiring-soon': isExpiringSoon(s.ngayKetThuc) }">
                  {{ d(s.ngayKetThuc) }}
                </span>
              </td>
              <td class="cell-price">{{ price(s.giaTien) }}</td>
              <td>
                <span class="pay-badge" :class="s.stripe_subscription_id ? 'pay-paid' : 'pay-unpaid'">
                  {{ s.stripe_subscription_id ? 'Đã thanh toán' : 'Chưa thanh toán' }}
                </span>
              </td>
              <td>
            ...
            <tr v-if="!loading && !subs.length">
              <td colspan="8" class="empty">Không có subscription nào</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="table-foot">
        ...
        <div class="pagination">
          ...
          <button :disabled="filters.page >= meta.totalPages" @click="filters.page++; fetch()">Tiếp →</button>
        </div>
      </div>
    </div>
          ...
          <div class="modal-body">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item">
                ...
                <span class="pay-badge" :class="selected.stripe_subscription_id ? 'pay-paid' : 'pay-unpaid'">
                  {{ selected.stripe_subscription_id ? 'Đã thanh toán' : 'Chưa thanh toán' }}
                </span>
              </div>
              <div class="detail-item">
              ...
              <div class="detail-item">
                ...
                <div class="detail-val" :class="{ 'text-warn': isExpiringSoon(selected.ngayKetThuc) }">{{ d(selected.ngayKetThuc) }}</div>
              </div>
              <div class="detail-item">
              ...
              <div class="detail-item" v-if="selected.stripe_subscription_id">
                <div class="detail-lbl">Stripe Session</div>
            ...
            <div class="timeline" v-if="selected.created_at">
              ...
              <div class="timeline-item" v-if="selected.stripe_subscription_id">
                <div class="tl-dot tl-dot--blue"></div>
                <div class="tl-body">
                  <div class="tl-action">Thanh toán thành công qua Stripe</div>
              ...
              <div class="timeline-item" v-if="selected.trangThai === 'expired'">
                ...
                <div class="tl-body">
                  <div class="tl-action">Đã hết hạn</div>
...
const summary = computed(() => {
  ...
  return [
    { label: 'Tổng', value: meta.value.total, color: '#1a2e1a' },
    { label: 'Đang hoạt động', value: all.filter(s => s.trangThai === 'active').length, color: '#3d8c3d' },
    { label: 'Hết hạn', value: all.filter(s => s.trangThai === 'expired').length, color: '#dc2626' },
    { label: 'Chờ thanh toán', value: all.filter(s => !s.stripe_subscription_id).length, color: '#d97706' },
  ]
})

let timer: any
function debounce() { clearTimeout(timer); timer = setTimeout(fetch, 400) }
...
.sum-card { background: white; border: 1px solid #e2ede2; border-radius: 12px; padding: 16px 18px; }
...
.cell-sub { font-size: 11px; color: #94a894; }
.cell-date { font-size: 12.5px; color: #6b836b; white-space: nowrap; }
.cell-price { font-size: 13px; font-weight: 500; color: #1a2e1a; white-space: nowrap; }
...
.pay-badge { display: inline-flex; padding: 3px 8px; border-radius: 20px; font-size: 11.5px; font-weight: 500; white-space: nowrap; }
.pay-paid { background: #eef7ee; color: #2d6e2d; }
.pay-unpaid { background: #fffbeb; color: #b45309; }
