<template>
  <div class="page">
    ...
    <div class="tabs">
      ...
      <button class="tab" :class="{ 'tab--active': activeTab === 'types' }" @click="activeTab = 'types'">
    ...
    <template v-if="activeTab === 'kiosks'">
      <div class="filter-bar">
        <div class="search-wrap">
          ...
          <input v-model="kFilters.q" placeholder="Tìm mã, vị trí..." @input="debouncedFetchK" />
        </div>
        ...
        <select v-model="kFilters.zone_id" @change="fetchKiosks">
          ...
          <option v-for="z in zonesFiltered" :key="z.zone_id" :value="z.zone_id">{{ z.tenKhu }}</option>
        </select>
        ...
        <button class="btn-outline" @click="resetKFilters">Reset</button>
      </div>
      ...
      <div class="table-panel">
            ...
            <tbody>
              ...
              <template v-else>
                <tr v-for="k in kiosks" :key="k.kiosk_id">
                  ...
                  <td><span class="badge" :class="statusClass(k.trangThai)">{{ statusLabel(k.trangThai) }}</span></td>
                  <td>
                    <div class="action-btns">
                      ...
                      <button v-if="k.trangThai === 'available'" class="icon-btn icon-btn--blue" title="Phân quầy" @click="openAssignFromKiosk(k)">
        ...
        <div class="table-foot">
          <span class="total-label">{{ kMeta.total }} kết quả</span>
          <div class="pagination" v-if="kMeta.totalPages > 1">
            <button :disabled="kPg.page <= 1" @click="changeKPage(kPg.page-1)">← Trước</button>
            <span>{{ kPg.page }} / {{ kMeta.totalPages }}</span>
            <button :disabled="kPg.page >= kMeta.totalPages" @click="changeKPage(kPg.page+1)">Tiếp →</button>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-if="activeTab === 'types'">
      <div class="filter-bar">
        <div class="search-wrap">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input v-model="tFilters.q" placeholder="Tìm loại kiosk..." @input="debouncedFetchT" />
        </div>
      </div>
      <div class="table-panel">
        <div class="table-wrap">
        ...
        <div class="table-foot">
          <span class="total-label">{{ tMeta.total }} loại</span>
          <div class="pagination" v-if="tMeta.totalPages > 1">
            <button :disabled="tPg.page <= 1" @click="changeTPage(tPg.page-1)">← Trước</button>
            <span>{{ tPg.page }} / {{ tMeta.totalPages }}</span>
            <button :disabled="tPg.page >= tMeta.totalPages" @click="changeTPage(tPg.page+1)">Tiếp →</button>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-if="activeTab === 'assignments'">
      <div class="filter-bar">
        <div class="search-wrap">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input v-model="aFilters.q" placeholder="Tìm tiểu thương, kiosk..." @input="debouncedFetchA" />
        </div>
                ...
                <tr v-for="a in assignments" :key="a.assignment_id">
                  <td>
                    <div class="cell-with-avatar">
                      <div class="merchant-avatar">{{ initials(a.merchantName) }}</div>
                      <div>
      ...
      <div v-if="showKioskModal" class="modal-overlay" @click.self="showKioskModal = false">
        <div class="modal">
          <div class="modal-header">
            ...
            <button class="modal-close" @click="showKioskModal = false">
            ...
            <div class="form-row">
              ...
              <div class="field">
                ...
                <select v-model="kForm.zone_id">
                  ...
                  <option v-for="z in kFormZones" :key="z.zone_id" :value="z.zone_id">{{ z.tenKhu }}</option>
                </select>
              </div>
            </div>
            <div class="form-row">
              ...
              <div class="field">
                ...
                <select v-model="kForm.type_id">
                  ...
                  <option v-for="t in allTypes" :key="t.type_id" :value="t.type_id">{{ t.tenLoai }}</option>
                </select>
              </div>
            </div>
            <div class="form-row">
      ...
      <div v-if="statusModal" class="modal-overlay" @click.self="statusModal = false">
        <div class="modal modal--sm">
          <div class="modal-header">
            ...
            <button class="modal-close" @click="statusModal = false">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
          <div class="modal-body">
      ...
      <div v-if="showTypeModal" class="modal-overlay" @click.self="showTypeModal = false">
        <div class="modal modal--sm">
          <div class="modal-header">
            ...
            <button class="modal-close" @click="showTypeModal = false">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
          <div class="modal-body">
            ...
            <div class="field">
              ...
              <div class="tenant-list-wrap">
                <div v-for="m in filteredMerchants" :key="m.merchant_id"
                  class="tenant-option"
                  :class="{ 'tenant-option--selected': aForm.merchant_id === m.merchant_id }"
                  @click="aForm.merchant_id = m.merchant_id"
                >
                  <div class="merchant-avatar sm">{{ initials(m.hoTen) }}</div>
                  <div class="to-info">
                    <div class="to-name">{{ m.hoTen }}</div>
                    <div class="to-email">{{ m.soDienThoai }}</div>
                  </div>
...
onMounted(async () => {
  await Promise.all([
    ...
    api.get('/kioskType', { params: { limit: 100 } }).then(r => allTypes.value = r.data.data || []).catch(() => {}),
    api.get('/merchant', { params: { limit: 500, trangThai: 'active' } }).then(r => merchants.value = r.data.data || []).catch(() => {}),
  ])
  try { const s = await api.get('/plan_subscription'); if (s.data?.gioiHanSoKiosk) quota.value.max = s.data.gioiHanSoKiosk } catch {}
  fetchKiosks()
  fetchTypes()
})
...
const kFilters = reactive({ q: '', market_id: route.query.market_id || '', zone_id: route.query.zone_id || '', trangThai: '' })
...
const zonesFiltered = computed(() => kFilters.market_id ? allZones.value.filter(z => z.market_id == kFilters.market_id) : allZones.value)
const kFormZones = computed(() => kForm.market_id ? allZones.value.filter(z => z.market_id == kForm.market_id) : allZones.value)
const availableKiosks = computed(() => kiosks.value.filter(k => k.trangThai === 'available'))

let kDebounce: any
function debouncedFetchK() { clearTimeout(kDebounce); kDebounce = setTimeout(() => { kPg.page = 1; fetchKiosks() }, 400) }
...
function changeKPage(p: number) { kPg.page = p; fetchKiosks() }

function openCreate() {
  if (activeTab.value === 'kiosks') { editingKiosk.value = null; Object.assign(kForm, { maKiosk: '', zone_id: '', market_id: '', type_id: '', viTri: '', dienTich: null }); kFormError.value = ''; showKioskModal.value = true }
  else if (activeTab.value === 'types') { editingType.value = null; Object.assign(tForm, { tenLoai: '', moTa: '' }); tFormError.value = ''; showTypeModal.value = true }
  else { openAssignModal() }
}

function openEditKiosk(k: any) {
  editingKiosk.value = k
  const zone = allZones.value.find(z => z.zone_id === k.zone_id)
...
function openAssignFromKiosk(k: any) { aForm.kiosk_id = k.kiosk_id; openAssignModal() }

function openStatusModal(k: any) { statusTarget.value = k; newStatus.value = k.trangThai === 'occupied' ? 'available' : k.trangThai; statusModal.value = true }

async function submitStatus() {
  if (!statusTarget.value) return
  saving.value = true
  try { await api.patch(`/kiosk/${statusTarget.value.kiosk_id}/status`, { trangThai: newStatus.value }); statusTarget.value.trangThai = newStatus.value; statusModal.value = false }
...
const tMeta = ref({ total: 0, totalPages: 1 })
const tPg = reactive({ page: 1, limit: 10 })
const tFilters = reactive({ q: '' })
...
let tDebounce: any
function debouncedFetchT() { clearTimeout(tDebounce); tDebounce = setTimeout(() => { tPg.page = 1; fetchTypes() }, 400) }

async function fetchTypes() {
  tLoading.value = true
  try {
    const p: any = { page: tPg.page, limit: tPg.limit }
    if (tFilters.q) p.q = tFilters.q
...
function changeTPage(p: number) { tPg.page = p; fetchTypes() }
...
const aFilters = reactive({ q: '', trangThai: 'active' })
...
const filteredMerchants = computed(() => {
  ...
  return merchants.value.filter(m => m.hoTen?.toLowerCase().includes(q) || m.soDienThoai?.includes(q))
})

let aDebounce: any
function debouncedFetchA() { clearTimeout(aDebounce); aDebounce = setTimeout(() => { aPg.page = 1; fetchAssignments() }, 400) }

async function fetchAssignments() {
  aLoading.value = true
  try {
    const p: any = { page: aPg.page, limit: aPg.limit }
    if (aFilters.q) p.q = aFilters.q
...
function resetAFilters() { aFilters.q = ''; aFilters.trangThai = 'active'; aPg.page = 1; fetchAssignments() }
...
async function submitAssign() {
  ...
  try {
    await api.post('/kioskAssignment', { kiosk_id: Number(aForm.kiosk_id), merchant_id: Number(aForm.merchant_id), ngayBatDau: aForm.ngayBatDau })
...
async function endAssignment(a: any) {
  ...
  try { await api.patch(`/kioskAssignment/${a.assignment_id}/end`); fetchAssignments(); fetchKiosks() }
  catch (e: any) { alert(e.response?.data?.message || 'Lỗi') }
}
...
function statusClass(s: string) { return { available: 'badge--green', occupied: 'badge--blue', maintenance: 'badge--amber', locked: 'badge--red' }[s] || 'badge--gray' }
function statusLabel(s: string) { return { available: 'Trống', occupied: 'Đang thuê', maintenance: 'Bảo trì', locked: 'Khóa' }[s] || s }
</script>
...
.tab--active { background: #3d8c3d; color: #1a3d00; font-weight: 600; }
...
.merchant-avatar { width: 32px; height: 32px; background: #7c3aed; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: white; flex-shrink: 0; }
.merchant-avatar.sm { width: 28px; height: 28px; font-size: 10px; }
...
.to-email { font-size: 11.5px; color: #94a894; }
