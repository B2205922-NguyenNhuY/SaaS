<template>
  <div class="sa-shell" :class="{ collapsed: sidebarCollapsed }">
    ...
    <div class="mobile-overlay" @click="sidebarCollapsed = true" />
  </div>
</template>
...
const navGroups: NavGroup[] = [
  {
    label: 'Tổng quan',
    items: [{ to: '/super-admin/dashboard', label: 'Dashboard', icon: iconGrid() }]
  },
  {
    label: 'Quản lý Tenant',
    items: [{ to: '/super-admin/tenants', label: 'Danh sách Tenant', icon: iconBuilding() }]
  },
  {
    label: 'Gói cước',
    items: [
  ...
  {
    label: 'Báo cáo & Log',
    items: [{ to: '/super-admin/audit-logs', label: 'Audit Log', icon: iconLog() }]
  },
  {
    label: 'Hệ thống',
    items: [
...
.sa-shell.collapsed .logo-img { width: 32px; height: 32px; }
...
.mobile-menu-btn {
  display: none;
  background: none; border: none;
  color: #4a654a; cursor: pointer; padding: 4px;
  flex-shrink: 0;
}
...
.breadcrumb-root { color: #94a894; flex-shrink: 0; }
.breadcrumb-current { color: #1a2e1a; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.breadcrumb svg { color: #b0c4b0; flex-shrink: 0; }
...
.topbar-avatar {
  width: 34px; height: 34px;
  background: #3d8c3d; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  color: white; font-size: 12px; font-weight: 600;
...
@media (max-width: 768px) {
  ...
  .sa-shell:not(.collapsed) .sa-sidebar {
    transform: translateX(0);
  }

  .sa-shell:not(.collapsed) .mobile-overlay {
    display: block;
  }
}
</style>