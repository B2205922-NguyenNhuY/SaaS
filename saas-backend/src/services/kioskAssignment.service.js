...
// PUT /kiosk_assignments/:id/end
// - set ended
// - ngayKetThuc = body.ngayKetThuc || today
// - set kiosk available
exports.end = async (tenant_id, assignment_id, body) => {
  let ngayKetThuc = body.ngayKetThuc;
  if (!ngayKetThuc) ngayKetThuc = todayYMD();
  ...
  try {
    ...
    // update assignment ended + ngayKetThuc
    await conn.query(
    ...
    // set kiosk available (chỉ đổi nếu đang occupied)
    await conn.query(
      `UPDATE kiosk
